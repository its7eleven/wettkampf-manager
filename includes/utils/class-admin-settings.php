<?php
/**
 * Email management utility - ERWEITERT mit Multi-E-Mail Support für Auto-Export
 */
...
class EmailManager {
    ...
    /**
     * Send automatic export email - ERWEITERT mit Multi-E-Mail Support
     */
    public function send_automatic_export($wettkampf, $recipient_emails = null) {
        WettkampfHelpers::log_error('send_automatic_export: Start für Wettkampf ' . $wettkampf->name);
        
        // Get export emails - either passed or from settings
        if ($recipient_emails === null) {
            if (class_exists('AdminSettings')) {
                $admin_settings = new AdminSettings();
                $recipient_emails = $admin_settings->get_export_emails();
            } else {
                // Fallback: Parse from option
                $export_email_text = WettkampfHelpers::get_option('export_email', '');
                $recipient_emails = $this->parse_email_list($export_email_text);
            }
        }
        ...
        WettkampfHelpers::log_error('send_automatic_export: Sende an ' . count($recipient_emails) . ' E-Mail-Adressen: ' . implode(', ', $recipient_emails));
        ...
        $message = $this->build_export_message($wettkampf, count($recipient_emails));
        ...
        foreach ($recipient_emails as $recipient_email) {
            WettkampfHelpers::log_error('Sende Export an: ' . $recipient_email);
            
            $result = $this->send_email($recipient_email, $subject, $message, array($temp_file));
            
            if ($result) {
                $success_count++;
                WettkampfHelpers::log_error('Export erfolgreich gesendet an: ' . $recipient_email);
            } else {
                $failed_recipients[] = $recipient_email;
                WettkampfHelpers::log_error('Export fehlgeschlagen für: ' . $recipient_email);
            }
        }
        ...
        // Log overall result
        if ($success_count > 0) {
            WettkampfHelpers::log_error('Automatic export sent for competition ' . $wettkampf->name . ' to ' . $success_count . '/' . count($recipient_emails) . ' recipients');
        }
        
        if (!empty($failed_recipients)) {
            WettkampfHelpers::log_error('Failed recipients: ' . implode(', ', $failed_recipients));
        }
    ...
    /**
     * Build export email message - ERWEITERT mit Multi-E-Mail Info
     */
    private function build_export_message($wettkampf, $recipient_count = 1) {
        $message = "Hallo,\n\n";
        ...
        if ($recipient_count > 1) {
            $message .= "Diese E-Mail wurde an " . $recipient_count . " konfigurierte E-Mail-Adressen gesendet.\n";
        }
    ...
    /**
     * Parse email list from text (one per line)
     */
    private function parse_email_list($email_text) {
        if (empty($email_text)) {
            return array();
        }
    ...
    private function send_email($to, $subject, $message, $attachments = array()) {
        ...
        if ($result) {
            WettkampfHelpers::log_error('send_email: E-Mail erfolgreich gesendet an ' . $to);
        } else {
            WettkampfHelpers::log_error('send_email: FEHLER beim E-Mail-Versand an ' . $to);
